...
for i = 1:m
    thisY = (c==y(i,1));
    J = J + sum(-thisY.*log(a3(i,:))-(1-thisY).*log(1-a3(i,:)));
    delta_3 = (a3(i,:) - thisY)';
...
%Part two: regularized cost function
sum_add = sum(Theta1(:).^2) - sum(Theta1(:,1).^2) + sum(Theta2(:).^2) - sum(Theta2(:,1).^2);

J = J + lambda / (2.0 * m) * sum_add;
